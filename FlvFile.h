...
class FlvHeader
{
    ...
    FlvHeader():signature_(NULL), data_(NULL) {
    }
    ~FlvHeader() {
       if (signature_) {
           delete[] signature_;
...
public:
    ...
    int init(const std::string& fileName, const char* h264_file = NULL, const char* aac_file = NULL);
    int destory();
...
private:
    ...
    // 创建tag
    Tag *CreateTag(unsigned char *pBuf, size_t nLeftLen);
